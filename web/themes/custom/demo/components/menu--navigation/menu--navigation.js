...
(function ($, Drupal) {
  Drupal.behaviors.navigationControl = {
    attach: function (context) {
      once('navigationControl', '#navigationControl', context).forEach(function (menu) {
        /**
         * Accessible Menu
        ...
        function supportMobileMenuControl(event) {
          if (event.key == 'Escape') {
            let isTopLevel = $(event.target).parent().parent().attr('data-depth') == 0;
            if (isTopLevel) {
              closeMobile();
            }
          }
        ...
        $(window).on('click', function (e) {
          ...
          if ($('body').hasClass('js-prevent-scroll')) {
            ...
            if (!navMenu[0].contains(e.target) && navMenu.parent()[0] !== e.target) {
              // Close Menu
              $('body').removeClass('js-prevent-scroll');
              $(mobileNavButton).removeClass('checked');
              $(mobileNavButton).attr('aria-expanded', 'false');
            ...
            switch (key) {
              ...
              case 'ArrowLeft':
                ...
                if ($(prev).find('.menu__link').length > 0) {
                  ...
                  let prev = $(targetParent).find(' > .menu__item > .menu__link');
              ...
              case 'ArrowRight':
                ...
                if ($(next).find('.menu__link').length > 0) {
                  ...
                  //  Find relative parent to current
                  let targetParent = $(target).parent().parent()[0];
                  let next = $(targetParent).find(' > .menu__item > .menu__link');
        ...
        class MenuButtons extends MenuLinks {
          constructor(domNode) {
            ...
            this.menuNode.querySelectorAll('.menu__link:not(button)').forEach(link => {
            ...
            let nodes = $(domNode).find(menuDepth + ' > li.menu__item > *').not('ul');
            for (let i = 0; i < nodes.length; i++) {
              let menuitem = nodes[i];
              this.menuitemNodes.push(menuitem);
              if (!this.firstMenuitem) {
            ...
            switch (key) {
              ...
              case 'ArrowDown':
                ...
                if (this.isOpen()) {
                  ...
                  if ($(next).find('.menu__link').length > 0) {
                    ...
                    let next = $("ul[data-depth='0'] > .menu__item > .menu__link");
                    // Wrap to first element in list
                    next[0].focus();
                  }
                  flag = true;
              ...
              case 'ArrowLeft':
                ...
                if ($(prev).find('.menu__link').length > 0) {
                  ...
                  let prev = $("ul[data-depth='0'] > .menu__item:not(:hidden) > .menu__link");
                  // Navigate to last list element
                  prev[prev.length - 1].focus();
                }
                break;
          ...
          onMenuitemKeydown(event) {
            ...
            switch (key) {
              ...
              case 'ArrowLeft':
                ...
                let prev = $(target).closest("ul[data-depth='0'] > .menu__item.menu__item--expanded").prev();
                if ($(prev).find('.menu__link').length > 0) {
                  //Move to previous element
                  $(prev).find('.menu__link').focus();
                } else {
                  let prev = $("ul[data-depth='0'] > .menu__item > .menu__link");
                  // Navigate to last list element
                  prev[prev.length - 1].focus();
                }
                break;
              ...
              case 'ArrowRight':
                ...
                let next = $(target).closest("ul[data-depth='0'] > .menu__item.menu__item--expanded").next();
                if ($(next).find('.menu__link').length > 0) {
                  //Move to next element
                  $(next).find('.menu__link').focus();
                } else {
              ...
              case 'Tab':
                ...
                if (event.shiftKey) {
                  ...
                  let expanded = $(target).parent().parent().parent().hasClass('menu__item--expanded');
                  ...
                  let menuLast = $(target).closest("ul[data-depth='1']").find('> li:last-child');
        ...
        // Main Menu Items
        const menuItems = $(menu).find(".c-menu--navigation ul[data-depth='0'] > li");

        // Utility menu
        const utilityMenuItems = $(menu).find(".c-menu ul[data-depth='0'] > li");
          ...
          for (let i = 0; i < menuItems.length; i++) {
            ...
            if (menuItems[i].querySelector('button')) {
              ...
              if (nextLevel.length > 0) {
                const nextLevelItems = $(menuItems[i]).find('.menu__item--expanded');
  ...
  Drupal.behaviors.ariaControls = {
    attach: function (context) {
      once('ariaControls', '#navigationControl', context).forEach(function (menu) {
        // Find top level menu buttons
        const buttons = $(menu).find('ul[data-depth].menu > li button.menu__link');
        ...
        buttons.each(function () {
          ...
          $(this).attr('aria-haspopup', 'true').attr('aria-controls', `panel-${id}`).next().attr('id', `panel-${id}`).attr('aria-label', $(this).text());
        });
      });
    }
  };
